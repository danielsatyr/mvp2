...
export function calculatePathways(data, occupations) {
  ...
  const eduPointsMap = { doctorate:20, bachelor:15, diploma:10, assessed:10, none:0 };
  const eduPts = eduPointsMap[eduQualification] || 0;
  points += eduPts;

  // 5) Specialist education
  if (specialistQualification === 'master_research' || specialistQualification === 'doctoral_research') {
    points += 10;
  }
  ...
  // 12) Crear breakdown
  const breakdown = {
    age: ageNum >= 18 && ageNum < 25 ? 25 : ageNum >= 25 && ageNum < 33 ? 30 : ageNum >= 33 && ageNum < 40 ? 25 : ageNum >= 40 && ageNum < 45 ? 15 : 0,
    english: ingles === 'Superior' ? 20 : ingles === 'Proficient' ? 10 : 0,
    overseas: ovExp >= 8 ? 15 : ovExp >= 5 ? 10 : ovExp >= 3 ? 5 : 0,
    australian: ausExp >= 8 ? 20 : ausExp >= 5 ? 15 : ausExp >= 3 ? 10 : ausExp >= 1 ? 5 : 0,
    education: eduPts,
    specialist: (specialistQualification==='master_research'||specialistQualification==='doctoral_research')?10:0,
